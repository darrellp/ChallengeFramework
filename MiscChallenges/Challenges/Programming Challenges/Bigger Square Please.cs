	...
	public static partial class ChallengeClass
	{
		...
		public class BiggerSquare : IChallenge
		{
			public string Solve(StringReader stm)
			{
				...
				{
					var puzzle = int.Parse(stm.ReadLine());
					SolvePuzzle(puzzle, ret);
				}
				// ReSharper restore AssignNullToNotNullAttribute
				return ret.ToString();
			}

			private void SolvePuzzle(int puzzle, StringBuilder strBuilder)
			{
				var initboard = new Board868(puzzle, new Board868(puzzle), 0);
				var res = initboard.Solve(puzzle * puzzle);
				strBuilder.Append(res.Count + Environment.NewLine);
				foreach (var board in res)
				{
					strBuilder.Append(string.Format("{0} {1} {2}" + Environment.NewLine, board.Row, board.Col, board.SquareSize));
				}
			}
