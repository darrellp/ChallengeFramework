...
namespace MiscChallenges.Challenges
{
    ...
    {
        [Challenge("Advent of Code 2016", "Balance Bots (10)", "https://adventofcode.com/2016/day/10")]
        public class BalanceBots : IChallenge
        {
            public void Solve()
            {
                var input = ReadAll();
            ...
            void GiveTo(int bot, int val, int[] firstValues, Dictionary<int, Give> gives, int[] outs)
            {
                if (firstValues[bot] < 0)
                {
                    firstValues[bot] = val;
                }
                else
                {
                    if (gives.ContainsKey(bot))
                    {
                        var give = gives[bot];

                        int low = Math.Min(val, firstValues[bot]);
                        int high = Math.Max(val, firstValues[bot]);
                        ...
                        if (give.LowIsOutput)
                        {
                            outs[give.LowBot] = low;
                        }
                        else
                        {
                            GiveTo(give.LowBot, low, firstValues, gives, outs);
                        }
                        if (give.HighIsOutput)
                        {
                            outs[give.HighBot] = high;
                        }
                        else
                        {
                            GiveTo(give.HighBot, high, firstValues, gives, outs);
                        }
                    }
                }
            }
            ...
            struct Give
            {
                public readonly bool HighIsOutput;
                public readonly int HighBot;
                public readonly bool LowIsOutput;
                public readonly int LowBot;

                public Give(int low, bool lowIsOutput, int high, bool highIsOutput)
                {
                    HighBot = high;
