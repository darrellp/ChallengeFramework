...
namespace MiscChallenges.Challenges
{
    ...
    {
        [Challenge("Advent of Code 2016", "Explosives in Cyberspace (9)", "https://adventofcode.com/2016/day/9")]
        public class ExplosivesInCyberspace : IChallenge
        {
            public void Solve()
            {
                var input = ReadLine();

                var output1 = DecompressCount(input);
                WriteLine(output1);
                var output2 = DecompressCountRec(input);
                WriteLine(output2);
            }

            int DecompressCount(string s)
            {
                var position = 0;
                var eof = false;
                var ret = 0;
                ...
                while (!eof)
                {
                    ...
                    {
                        ret++;
                        eof = ++position >= s.Length;
                        continue;
                    }
                    var match = Regex.Match(s.Substring(position));
                    if (!match.Success)
                    {
                        ret++;
                        eof = ++position >= s.Length;
                    ...
                    var chars = int.Parse(match.Groups["chars"].Value);
                    var rep = int.Parse(match.Groups["rep"].Value);
            ...
            long DecompressCountRec(string s)
            {
                var position = 0;
                var eof = false;
                long ret = 0;
                ...
                while (!eof)
                {
                    ...
                    var chars = int.Parse(match.Groups["chars"].Value);
                    long rep = int.Parse(match.Groups["rep"].Value);
                    position += match.Value.Length;
                    long repCount = DecompressCountRec(s.Substring(position, chars));
                    ret += repCount * rep;
                    position += chars;
                    eof = position >= s.Length;
                }
            ...
            static readonly string Search = Begin + '('.Esc() + Integer("chars") + "x" + Integer("rep") + ')'.Esc();
            static readonly Regex Regex = new Regex(Search);
