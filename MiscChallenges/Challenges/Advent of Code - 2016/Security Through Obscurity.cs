...
namespace MiscChallenges.Challenges
{
    ...
    {
        [Challenge("Advent of Code 2016", "Security Through Obscurity (4)", "https://adventofcode.com/2016/day/4")]
        public class SecurityThroughObscurity : IChallenge
        {
            public void Solve()
            {
                var input = ReadAll();
                var roomNames = input.Split("\n\r".ToCharArray(), StringSplitOptions.RemoveEmptyEntries);

                var output = roomNames
                    .Where(GoodName)
                    .Select(SectorId)
                    .Sum();
                WriteLine(output);

                var theRoom = roomNames
                    .Where(GoodName)
                    .First(s => Decrypt(s) == "northpole object storage");

                output = SectorId(theRoom);
                WriteLine(output);
            }

            private bool GoodName(string s)
            {
                var ln = s.Length;
                var computedCheckSumChars = s
                    .Substring(0, ln - 7)
                    .Where(Char.IsLower)
                    .GroupBy(c => c)
                    .Select(i => new Tuple<int, char>(-i.Count(), i.Key))
                    .OrderBy(v => v)
                    .Select(v => v.Item2)
                    .Take(5)
                    .ToArray();
                var computedCheckSum = new String(computedCheckSumChars);
                var checkSum = s.Substring(ln - 6, 5);
                return checkSum == computedCheckSum;
            }

            string EncryptedName(string s)
            {
            ...
            {
                var sectorId = SectorId(s);
                return new string(
                    EncryptedName(s)
                        .Select(c => c == '-' ? ' ' : (char)('a' + (c - 'a' + sectorId) % 26))
                        .ToArray()
                );
            }

            int SectorId(string s)
            {
