    ...
    public static partial class ChallengeClass
    {
        ...
        public class InternetProtocolVersion7 : IChallenge
        {
            public void Solve()
            {
                ...
                var abba = LowerLetter.Named("first") + LowerLetter.Named("second") +
                           BRef("second") + BRef("first") +
                           (BRef("first") + BRef("first")).NegLookBehind();
                var abbaNegSearch = '['.Esc() + Not(']'.Esc()).Rep(0) + abba;
                var abbaPosSearch = Begin.OrAnyOf("]") + Not('['.Esc()).Rep(0) + abba;
                var regexNeg = new Regex(abbaNegSearch, RegexOptions.Compiled);
                var regexPos = new Regex(abbaPosSearch, RegexOptions.Compiled);

                var output = inputLines.Count(l =>
                        !regexNeg.IsMatch(l) && regexPos.IsMatch(l));
                WriteLine(output);

                output = inputLines.Count(CheckSSL);
                WriteLine(output);
            }
            bool CheckSSL(string line)
            {
                var abaOutOfBrackets = new HashSet<string>();

                var sections = line.Split("[]".ToCharArray());
                var outsideBrackets = Enumerable.Range(0, sections.Length).Where(i => (i & 1) == 0).Select(i => sections[i]).ToArray();
                var insideBrackets = Enumerable.Range(0, sections.Length).Where(i => (i & 1) == 1).Select(i => sections[i]).ToArray();

                foreach (var outside in outsideBrackets)
                {
                    for (var i = 0; i < outside.Length - 2; i++)
                    {
                        if (outside[i] == outside[i + 2] && outside[i] != outside[i + 1])
                        {
                            abaOutOfBrackets.Add(outside.Substring(i, 3));
                        }
                    }
                }

                foreach (var inside in insideBrackets)
                {
                    for (var i = 0; i < inside.Length - 2; i++)
                    {
                        if (inside[i] == inside[i + 2] && inside[i] != inside[i + 1])
                        {
                            if (abaOutOfBrackets.Contains(new string(new[] { inside[i + 1], inside[i], inside[i + 1] })))
                            {
                                return true;
                            }
                        }
