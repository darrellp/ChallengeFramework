	...
	public static partial class ChallengeClass
	{
		...
		public class FerryLoadingIII : IChallenge
		{
			public string Solve(StringReader stm)
			{
				...
				{
					var ferryCase = new FerryCase(stm);

					ferryCase.SolveCase(ret, fFirstCase);
					fFirstCase = false;
				}
				return ret.ToString();
			}
...
";
		...
		class FerryCase
		{
			...
			public void SolveCase(StringBuilder ret, bool fFirstCase)
			{
				_ferryArrival = -1;
				...
				{
					ret.Append(_unloadTimes[iCar] + Environment.NewLine);
				}
			}
